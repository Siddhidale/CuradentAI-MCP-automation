...
async function login(page) {
  ...
  // Wait for URL to change from login page (explicit wait for navigation)
  await page.waitForURL((/** @type {URL} */ url) => !url.pathname.endsWith('/login'), { timeout: 30000 });

  // Wait for page to fully load after redirect
  await page.waitForLoadState('domcontentloaded');
  await page.waitForTimeout(1000); // Small buffer for any async operations
}

// Helper function to logout
async function logout(page) {
  const logoutButton = page.getByText(/logout/i).or(page.locator('[data-testid="logout"]'));
  if (await logoutButton.isVisible({ timeout: 3000 }).catch(() => false)) {
    await logoutButton.click();
    // Handle confirmation popup if present
    const confirmButton = page.getByRole('button', { name: /yes|confirm|logout/i });
...
test.describe('Patient Search', () => {
  ...
  test('CURADENTAI-2617: Search non-existing patient', async ({ page }) => {
    ...
    if (await searchInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      // Verify "No Patient Found" or similar message is displayed
      await expect(page.getByText(/no patients found|not found|no results|no data/i)).toBeVisible({ timeout: 5000 });
    }
  });
...
test.describe('Add Patient', () => {
  ...
  // Patient details (NO numbers in names)
  const firstName = 'Charlie';
  const lastName = 'Brown';
  const email = `charlie.${Date.now()}@yopmail.com`;
  ...
  // ✅ Assert SUCCESS POPUP (not toaster)
  const successPopup = page.getByRole('dialog');
  await expect(successPopup).toBeVisible({ timeout: 20000 });
  await expect(
    successPopup.getByText(/form sent successfully/i)
  ).toBeVisible();

  // Close popup if button exists
  const closeBtn = successPopup.getByRole('button', { name: /ok|close|done/i });
  ...
  await expect(
    page.getByText(new RegExp(`${firstName}\\s+${lastName}`, 'i'))
  ).toBeVisible({ timeout: 10000 });
});
...
test.describe('Patient Details', () => {
  ...
  test('CURADENTAI-2625: Patient details page', async ({ page }) => {
    /**
     * Description: Verify patient details page loads
     * Preconditions: Patient exists
     *
     * Steps:
     * 1. Click any existing patient record // Robinn Wilson
     * Expected Result: Details page opens
     */
    await login(page);
    ...
    if (await firstPatient.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const detailsPage = page.locator('[data-testid="patient-details"], .patient-details, main');
      await expect(detailsPage.first()).toBeVisible({ timeout: 5000 });
    }
  });
...
//      * Preconditions: Password updated
//      *
//      * Steps:
//      * 1. Logout → Login with new password
//      * Expected Result: Login successful
//      */
//     // Note: This test depends on CURADENTAI-2634
//     await page.goto(BASE_URL, { timeout: 60000 });
//     await page.waitForLoadState('domcontentloaded');
